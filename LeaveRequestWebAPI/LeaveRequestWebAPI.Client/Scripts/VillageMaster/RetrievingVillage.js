...
function LoadDistrictComboBox() {
    $.ajax({
        ...
        success: function (result) {
            var regencyId = $('#Districts_Id');
            $.each(result, function (i, Districts) {
                $("<option></option>").val(Districts.Id).text(Districts.Name).appendTo(regencyId);
            });
        }
    });
}
...
function Save() {
    ...
    district.Districts_Id = $('#Districts_Id').val();
    $.ajax({
        type: 'POST',
        url: "http://localhost:18565/api/Villages/",
        data: district,
...
function Edit() {
    ...
    district.id = $('#Id').val();
    district.name = $('#Name').val();
    district.Villages_Id = $('#Districts_Id').val();
    $.ajax({
        type: 'PUT',
        url: "http://localhost:18565/api/Villages/" + district.id,
        data: district,
        datatype: 'json',
        //success: function () {
        //    LoadIndexDistrict();
